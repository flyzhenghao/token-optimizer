...
- **`measure.py setup-daemon`**: Installs a macOS launchd daemon serving the dashboard at `http://localhost:24842/`. Starts on login, restarts on crash. Remove with `--uninstall`.
